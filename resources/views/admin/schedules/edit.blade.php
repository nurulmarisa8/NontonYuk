        ...
        <form method="POST" action="{{ route('admin.schedules.update', $schedule->id) }}">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="datetime-local" id="showtime" name="showtime" value="{{ old('showtime', \Carbon\Carbon::parse($schedule->showtime)->format('Y-m-d\TH:i')) }}" required class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500">
                    @error('showtime')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <select id="total_seats" name="total_seats" required class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500">
                        ...
                        <option value="15" {{ old('total_seats', $schedule->total_seats) == 15 ? 'selected' : '' }}>15 Seats</option>
                        <option value="30" {{ old('total_seats', $schedule->total_seats) == 30 ? 'selected' : '' }}>30 Seats</option>
                        <option value="50" {{ old('total_seats', $schedule->total_seats) == 50 ? 'selected' : '' }}>50 Seats</option>
                        <option value="100" {{ old('total_seats', $schedule->total_seats) == 100 ? 'selected' : '' }}>100 Seats</option>
                    </select>
                    @error('total_seats')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="number" id="price" name="price" value="{{ old('price', $schedule->price) }}" min="0" required class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500">
                    @error('price')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="mt-6 flex justify-end space-x-3">
                ...
                <button type="submit" class="px-4 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700">Update Schedule</button>
            </div>
        </form>
    </div>
</div>
